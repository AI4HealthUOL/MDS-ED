...
class TimeseriesDatasetCrops(torch.utils.data.Dataset):
    ...
    def _getitem(self, idx, start_idx_rel):
        ...
        if self.static:
            sample = (data, label, self.timeseries_df_static[df_idx])
...
class ToTensor(object):
    ...
    def __init__(self, transpose_data=True, transpose_label=False):
        # Swap channel and time axis for direct application of PyTorch's convolutions
    ...
    def __call__(self, sample):
        ...
        data = tuple(_to_tensor(x, self.transpose_data) for x in data) if isinstance(data, tuple) else _to_tensor(data, self.transpose_data)
        label = tuple(_to_tensor(x, self.transpose_label) for x in label) if isinstance(label, tuple) else _to_tensor(label, self.transpose_label)

        if all_out == 2:
            return data, label
        elif all_out == 3:
            static = tuple(_to_tensor(x) for x in static) if isinstance(static, tuple) else _to_tensor(static)
            return data, label, static

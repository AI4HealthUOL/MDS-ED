...
class MambaTSModel(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.encoder(x).permute(0,2,1) if self.transposed_input else self.encoder(x.permute(0,2,1))
        ...
        for layer, norm, dropout in zip(self.mamba_layers, self.norms, self.dropouts):
            ...
            if self.prenorm:
                # Prenorm
                z = norm(z) if self.layer_norm else norm(z.transpose(-1, -2)).transpose(-1, -2)
            
            # Apply mamba block: we ignore the state input and output
            z  = layer(z)
            ...
            if not self.prenorm:
                # Postnorm
                x = norm(z) if self.layer_norm else norm(x.transpose(-1, -2)).transpose(-1, -2)
        ...
        return x # (B,d_model), (B,d_output), (B, d_output, L)

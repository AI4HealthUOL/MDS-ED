...
def multiclass_roc_curve(y_true, y_pred, classes=None, precision_recall=False):
    ...
    # Create a mask to exclude samples with -999 in y_true
    mask = (y_true != -999)    
        
    for i, c in enumerate(classes):
        if precision_recall:
            tpr[c], fpr[c], _ = precision_recall_curve(y_true[:, i][mask[:, i]], y_pred[:, i][mask[:, i]])
            roc_auc[c] = auc_prrc_uninterpolated(fpr[c], tpr[c])
        else:
            fpr[c], tpr[c], _ = roc_curve(y_true[:, i][mask[:, i]], y_pred[:, i][mask[:, i]])
    ...
    if precision_recall:
        tpr["micro"], fpr["micro"], _ = precision_recall_curve(y_true.ravel()[mask.ravel()], y_pred.ravel()[mask.ravel()])
        roc_auc["micro"] = auc_prrc_uninterpolated(fpr["micro"], tpr["micro"])
    else:
        fpr["micro"], tpr["micro"], _ = roc_curve(y_true.ravel()[mask.ravel()], y_pred.ravel()[mask.ravel()])
   ...
   # Compute macro-average curve and area (linear interpolation is incorrect for PRRC- therefore just for ROC)
    if not precision_recall:
        ...
        # 2. Then interpolate all curves at these points
        mean_tpr = None
        for c in classes:
    ...
    for c in classes:
        if np.isnan(roc_auc[c]):
            # Conservative choice: replace auc by 0.5 if it could not be calculated
            roc_auc_macro += 0.5
            macro_auc_nans += 1
        else:
